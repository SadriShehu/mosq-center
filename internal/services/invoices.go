...
func (s *invoicesService) GenerateInvoices(ctx context.Context, year int, neighbourhoodID string, limit, skip int64) ([]byte, error) {
	families, err := s.InvoicesRepository.NoPayment(ctx, year, neighbourhoodID, limit, skip)
	if err != nil {
		return nil, err
	}
